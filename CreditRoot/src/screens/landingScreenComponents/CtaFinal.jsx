            ...
            <div className={`relative bg-brand rounded-3xl px-5 py-10 text-center overflow-hidden transition-all duration-700 ${visible ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
                ...
                <div className="relative z-10">
                    ...
                    <h2 className="font-display font-black text-white mb-4 tracking-tight"
                        ...
                        Tu retiro empieza<br />
                        <em className="italic opacity-85">hoy mismo.</em>
                    </h2>

                    <p className="text-white/70 text-base leading-relaxed max-w-md mx-auto mb-10">
                        Sin banco, sin intermediarios. El contrato Soroban garantiza que
                        tus fondos están seguros y generando rendimiento real.
                    </p>

                    <div className="flex gap-3 justify-center flex-wrap items-center">
                        <button
                            className="bg-white text-brand font-bold px-8 py-3.5 rounded-xl hover:-translate-y-px hover:shadow-lg transition-all"
                            onClick={onRegister}>
                            Crear cuenta gratis →
                        </button>
                        <button
                            className="border-[1.5px] border-white/40 text-white font-semibold px-8 py-3.5 rounded-xl hover:bg-white/10 hover:-translate-y-px transition-all"
                            onClick={onLogin}>
                            Ya tengo cuenta
                        </button>
                    </div>
                    ...
                    <p className="text-white/40 text-xs mt-5">
                        Sin tarjeta de crédito · 100% on-chain
                    </p>
                </div>
            </div>
        </section>
